...
def lstm(state_mean_tm1, state_var_tm1, inpt_mean, inpt_var):
    ...
    x_mean, x_var = tanh(inpt_mean[:, :size], inpt_var[:, :size])
    gates_mean, gates_var = inpt_mean[:, size:], inpt_var[:, size:]

    ingate_mean, ingate_var = sigmoid(gates_mean[:, :size], gates_var[:, :size])
    forgetgate_mean, forgetgate_var = sigmoid(gates_mean[:, size:2 * size], gates_var[:, size:2 * size])
    outgate_mean, outgate_var = tanh(gates_mean[:, 2 * size:], gates_var[:, 2 * size:])
    ...
    state_post_mean, state_post_var = tanh(state_mean, state_var)
